...
- (IBAction)loginToSPiD:(id)sender {
    ...
    if ([email length] == 0) {
        ...
        [(AppDelegate *) [[UIApplication sharedApplication] delegate] showActivityIndicatorAlert:@"Logging in using SPiD\nPlease Wait..."];
        SPiDTokenRequest *tokenRequest = [SPiDTokenRequest userTokenRequestWithUsername:email password:password completionHandler:^(NSError *error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:title];
            });
        }];
        [tokenRequest start];
    }
