...
- (void)signUpToSPiD:(id)signUpToSPiD {
    ...
    if ([email length] == 0) {
        [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:@"Email is empty"];
    } else if ([password length] == 0) {
        [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:@"Password is empty"];
    } else {
...
- (void)createSPiDAccountWithEmail:(NSString *)email andPassword:(NSString *)password {
    [(AppDelegate *) [[UIApplication sharedApplication] delegate] showActivityIndicatorAlert:@"Creating SPiD account\nPlease Wait..."];
    [SPiDUser createAccountWithEmail:email password:password completionHandler:^(NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [(AppDelegate *) [[UIApplication sharedApplication] delegate] dismissAlertView];
            if (error) {
                if ([error.userInfo objectForKey:@"blocked"]) {
                    [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:[error.userInfo objectForKey:@"blocked"]];
                } else if ([error.userInfo objectForKey:@"exists"]) {
                    [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:[error.userInfo objectForKey:@"exists"]];
                } else if ([error.userInfo objectForKey:@"email"]) {
                    [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:[error.userInfo objectForKey:@"email"]];
                } else if ([error.userInfo objectForKey:@"password"]) {
                    [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:[error.userInfo objectForKey:@"password"]];
                } else {
                    NSString *errorString = nil;
                    NSArray *values = [error.userInfo allValues];
                    if ([values count] != 0)
                        errorString = [values objectAtIndex:0];
                    [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:errorString];
                }
            } else {
                //[self.navigationController dismissViewControllerAnimated:YES completion:nil];
                [(AppDelegate *) [[UIApplication sharedApplication] delegate] showAlertViewWithTitle:@"Successfully created SPiD account\nCheck your email for verification"];
